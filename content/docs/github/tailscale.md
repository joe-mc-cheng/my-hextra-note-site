...
用 NSSM 將 Flask 變成長駐服務，或者遇到 Windows 權限問題，請告訴我！

使用 NSSM (Non-Sucking Service Manager) 是 Windows 上最推薦的做法。它能將你的 .exe 封裝成 Windows 服務，實現開機自動啟動、當機自動重啟且完全隱藏黑視窗。
以下是具體的操作步驟：
------------------------------
## 1. 下載與安裝 NSSM

   1. 前往 NSSM 官網 下載壓縮檔。
   2. 解壓縮後，根據你的系統位元進入 win64 或 win32 資料夾。
   3. 將 nssm.exe 複製到你的 App 目錄（例如 C:\App\），或者加入系統路徑（PATH）。

------------------------------
## 2. 使用 GUI 界面建立服務
以管理員權限開啟 PowerShell 或 CMD，輸入以下指令：
```
.\nssm.exe install FlaskService
```
(註：FlaskService 是你自定義的服務名稱)
隨後會跳出設定視窗，請填入：

* Path (程式路徑): 選擇你打包好的執行檔，例如 C:\App\my_flask_app.exe。
* Startup directory (啟動目錄): 通常會自動填入 C:\App\。
* Arguments (參數): 如果啟動時需要參數（如 --port 5000）才填寫，否則留空。
* Details 頁籤: 可填寫服務的顯示名稱與描述。

設定完後點擊 Install service。
------------------------------
## 3. 設定服務啟動與管理
在 PowerShell 中使用以下指令啟動服務：
```
# 啟動服務
Start-Service FlaskService
# 檢查狀態
Get-Service FlaskService
# 停止服務
Stop-Service FlaskService
```
------------------------------
## 4. 配合 GitHub Actions 的部署腳本
現在你有服務了，GitHub Actions 的部署腳本（ssh-action 部分）可以改寫得更優雅：
```
      - name: 遠端部署與服務重啟
        uses: appleboy/ssh-action@v1.0.3
        with:
          ...
          key: ${{ secrets.SERVER_SSH_KEY }}
          script: |
            # 1. 停止舊服務 (NSSM 會處理剩餘進程)
            nssm stop FlaskService
            
            # 2. 覆蓋執行檔 (假設新檔已透過 scp 傳到 Temp)
            Copy-Item -Path "C:\Deploy\Temp\my_flask_app.exe" -Destination "C:\App\my_flask_app.exe" -Force
...
* 自動重啟: 如果你的 Flask 因為記憶體洩漏或錯誤崩潰，NSSM 會自動幫你重新拉起來。
* 日誌記錄: 在 NSSM GUI 的 I/O 頁籤，你可以設定 stdout 和 stderr 的路徑。這非常重要！你可以把 Flask 的 Print 輸出導向到 C:\App\logs\out.log，方便遠端排錯。
* 權限設定: 預設以 LocalSystem 執行，若 Flask 需要存取特定使用者的資料夾，可在 Log on 頁籤 修改帳號。
